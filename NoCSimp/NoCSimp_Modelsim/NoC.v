...
wire [DATA_WIDTH-1 : 0] dataInE, dataInW, dataInS, dataInN, dataInL; // 4 addres, 1 endbit
wire [DATA_WIDTH-1 : 0] dataOutE, dataOutW, dataOutS, dataOutN, dataOutL;
wire Inr_L, Inr_N, Inr_E, Inr_S, Inr_W;
wire Inw_L, Inw_N, Inw_E, Inw_S, Inw_W;
wire Outr_L, Outr_N, Outr_E, Outr_S, Outr_W;
wire Outw_L, Outw_N, Outw_E, Outw_S, Outw_W;
...
IP #(DATA_WIDTH, 4'b0101, FREQ) IPL (.DataOutIP(dataInL),.Outr(Inr_L),.Outw(Inw_L),.DataInIP(dataOutL),
                                     .Inr(Outr_L),.Inw(Outw_L),.clk(clk),.reset(reset));      
switch #(DATA_WIDTH, 4'b0101, 5) SW (.dataInL(dataInL),.dataInE(dataInE),.dataInW(dataInW),.dataInS(dataInS),.dataInN(dataInN),
